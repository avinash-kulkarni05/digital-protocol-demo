...
# Unicode superscript character → normal character mapping
...
class HTMLGridParser:
    ...
    def parse(self, html: str, table_id: str, page_number: int) -> TableGrid:
        ...
        for row in table.find_all('tr'):
            ...
            for cell in row.find_all(['td', 'th']):
                ...
                # Deterministically extract footnote markers from HTML
                cell_markers = self._extract_cell_markers_from_element(cell)
    ...
    def _extract_cell_markers_from_element(self, cell_element) -> List[str]:
        """Extract footnote markers from an HTML cell element deterministically.

        Finds markers from:
        - <sup> child tags (each alphanumeric character is a marker)
        - Unicode superscript characters in cell text
        """
        markers = set()

        # Extract from <sup> tags
        for sup in cell_element.find_all('sup'):
            sup_text = sup.get_text(strip=True)
            for char in sup_text:
                if char.isalnum():
                    markers.add(char.lower())

        # Extract from unicode superscript characters in the cell's text
        cell_text = cell_element.get_text()
        for sup_char, normal_char in UNICODE_SUPERSCRIPTS.items():
            if sup_char in cell_text:
    ...
    def _build_grid(
        ...
        # Build marker_to_cells index from deterministically extracted markers
        marker_to_cells: Dict[str, List[CellPosition]] = {}
        for cell in cells:
            for marker in cell.footnote_markers:
                marker_to_cells.setdefault(marker, []).append(cell.position)
        ...
        return TableGrid(
            ...
            marker_to_cells=marker_to_cells,
        )
