...
class ExtractionReviewGenerator:
    ...
    def _process_usdm_footnotes(
        ...
        for idx, fn in enumerate(raw_footnotes):
            ...
            # Convert appliesTo from backend flat array to structured format
            raw_applies = fn.get("appliesTo", [])
            applies_to = {"visits": [], "activities": [], "cells": []}
            if isinstance(raw_applies, list):
                seen_visits = set()
                seen_activities = set()
                for ref in raw_applies:
                    if isinstance(ref, str) and "@" in ref:
                        act_id, visit_id = ref.split("@", 1)
                        applies_to["cells"].append({"activityId": act_id, "visitId": visit_id})
                        if visit_id not in seen_visits:
                            applies_to["visits"].append(visit_id)
                            seen_visits.add(visit_id)
                        if act_id not in seen_activities:
                            applies_to["activities"].append(act_id)
                            seen_activities.add(act_id)
                    elif isinstance(ref, str):
                        # Standalone visit or activity ref
                        if ref.startswith("ENC-"):
                            if ref not in seen_visits:
                                applies_to["visits"].append(ref)
                                seen_visits.add(ref)
                        elif ref.startswith("ACT-"):
                            if ref not in seen_activities:
                                applies_to["activities"].append(ref)
                                seen_activities.add(ref)
            elif isinstance(raw_applies, dict):
                applies_to = raw_applies  # Already structured
