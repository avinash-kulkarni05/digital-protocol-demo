...
class PipelineConfig:
    ...
    # Pre-computed classification profile (skips Stage 0 if set)
    pre_computed_profile: Optional[Dict[str, Any]] = None
...
class InterpretationPipeline:
    ...
    async def run(
        ...
        # Pre-stage: Classify table structure for protocol-agnostic routing
        classify_start = time.time()
        if config.pre_computed_profile:
            # Use human-confirmed profile — skip LLM classification
            working_usdm["soaTableProfile"] = config.pre_computed_profile
            classify_duration = time.time() - classify_start
            result.stage_durations[0] = classify_duration
            result.stage_statuses[0] = "success"
            result.stages_completed += 1
            logger.info(
                f"[Stage 0] Table Classification - SKIPPED (using pre-computed profile): "
                f"{config.pre_computed_profile.get('tableStructureType', 'unknown')} "
                f"(confidence: {config.pre_computed_profile.get('confidence', 0):.2f})"
            )
            if self._progress_callback:
                try:
                    self._progress_callback(0, "Table Classification", "success")
                except Exception as cb_err:
                    logger.warning(f"Progress callback error: {cb_err}")
        else:
            try:
                from .soa_table_classifier import SOATableClassifier
                classifier = SOATableClassifier()
                table_profile = await classifier.classify(
                    working_usdm,
                    extraction_outputs=config.extraction_outputs,
                )
                working_usdm["soaTableProfile"] = table_profile
                classify_duration = time.time() - classify_start
                result.stage_durations[0] = classify_duration
                result.stage_statuses[0] = "success"
                logger.info(
                    f"[Stage 0] Table Classification - Completed in {classify_duration:.2f}s: "
                    f"{table_profile.get('tableStructureType', 'unknown')} "
                    f"(confidence: {table_profile.get('confidence', 0):.2f})"
                )
                if self._progress_callback:
                    try:
                        self._progress_callback(0, "Table Classification", "success")
                ...
                classify_duration = time.time() - classify_start
                result.stage_durations[0] = classify_duration
                result.stage_statuses[0] = "failed"
                working_usdm["soaTableProfile"] = {}
                logger.warning(f"[Stage 0] Table Classification - FAILED: {e}")
                result.warnings.append(f"Table classification failed: {e}")
        ...
        # Sync visits from encounters — stages like Stage 8 (Cycle Expansion) modify
        # encounters but not visits. The frontend reads visits, so keep them in sync.
        if "encounters" in working_usdm:
            working_usdm["visits"] = working_usdm["encounters"]
    ...
    async def _execute_stage(
        ...
        if stage == 1:
            ...
            result = await self._run_async_or_sync(
                handler.expand_cycles,
                usdm,
                extraction_outputs=config.extraction_outputs,
            )
            if result and result.expansions:
                handler.apply_expansions_to_usdm(usdm, result)
                logger.info(
                    f"           Applied {result.encounters_expanded} cycle expansions, "
                    f"created {result.encounters_created} encounters"
                )
            return result
