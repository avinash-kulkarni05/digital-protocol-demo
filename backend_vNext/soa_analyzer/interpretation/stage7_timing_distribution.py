...
class TimingDistributor:
    ...
    def _build_llm_prompt(self, timing_modifiers: List[str]) -> str:
        """Build prompt for LLM timing analysis with table classification context."""
        ...
        # Build table classification block from pre-stage classifier
        table_profile = getattr(self, "_table_profile", {})
        table_classification = "(No table classification available)"
        if table_profile and table_profile.get("tableStructureType"):
            guidance = table_profile.get("stageGuidance", {}).get("stage7_timing", "")
    ...
    async def distribute_timing(
        ...
        # Store table profile from classifier (if available) — used in _build_llm_prompt
        self._table_profile = usdm_output.get("soaTableProfile", {})
        if self._table_profile and self._table_profile.get("tableStructureType"):
            timing_guidance = self._table_profile.get("stageGuidance", {}).get("stage7_timing", "")
            logger.info(
                f"Table profile: {self._table_profile.get('tableStructureType')} "
                f"(confidence: {self._table_profile.get('confidence', 0):.2f})"
            )
            if timing_guidance:
                logger.info(f"Timing guidance: {timing_guidance}")
