...
from soa_analyzer.table_merge_analyzer import (
    ...
    MergeGroup,
    MergeType,
    combine_table_usdm_naive as combine_table_usdm,
)
...
class SOAExtractionPipeline:
    ...
    async def run_with_merge_analysis(
        ...
        try:
            ...
            # Phase 3b: Per-table 12-stage interpretation
            logger.info("\n[Phase 3b] PER-TABLE INTERPRETATION")
            logger.info("-" * 40)
            ...
            successful_tables = [ptr for ptr in per_table_results if ptr.success and ptr.usdm]
            for table_idx, ptr in enumerate(successful_tables):
                logger.info(f"  [{table_idx+1}/{len(successful_tables)}] Interpreting {ptr.table_id}...")

                try:
                    # Fresh pipeline per table to avoid cached state bleed
                    table_pipeline = InterpretationPipeline()

                    interp_config = InterpretationConfig(
                        protocol_id=f"{protocol_id}_{ptr.table_id}",
                        protocol_name=protocol_name,
                        extraction_outputs=effective_extraction_outputs,
                        gemini_file_uri=effective_gemini_uri,
                        skip_stage_11=True,
                        continue_on_non_critical_failure=True,
                        save_intermediate_results=True,
                        output_dir=output_path / "interpretation_stages" / ptr.table_id,
                    )

                    pipeline_result = await table_pipeline.run(ptr.usdm, interp_config)

                    if pipeline_result.final_usdm:
                        propagate_provenance(pipeline_result.final_usdm)
                        ptr.usdm = pipeline_result.final_usdm
                        logger.info(f"    {ptr.table_id}: {pipeline_result.get_summary()}")
                    else:
                        logger.warning(f"    {ptr.table_id}: No final USDM, keeping raw")

                except Exception as e:
                    logger.error(f"    {ptr.table_id} interpretation failed: {e}", exc_info=True)
                    # Continue with raw USDM for this table

            logger.info(f"  Phase 3b complete: {len(successful_tables)} tables interpreted")
            ...
            # If auto_confirm_merges, assemble timelines from already-interpreted per-table USDMs
            if auto_confirm_merges:
                logger.info("\n[Auto-Confirm] Assembling timelines from interpreted per-table USDMs...")

                # Build timeline output from already-interpreted per-table results (no LLM calls)
                all_usdm = []
                merge_group_results = []

                # Build lookup for quick access
                table_usdm_map = {ptr.table_id: ptr for ptr in per_table_results if ptr.success and ptr.usdm}

                for mg in merge_plan.merge_groups:
                    logger.info(f"  Assembling group {mg.id}: {mg.table_ids}")

                    # Build scheduleTimelines — each table is its own timeline (no dedup)
                    schedule_timelines = []
                    for tid in mg.table_ids:
                        ptr = table_usdm_map.get(tid)
                        if ptr and ptr.usdm:
                            schedule_timelines.append({
                                "timelineId": f"ST-{mg.id}-{tid}",
                                "sourceTableId": tid,
                                "usdm": ptr.usdm,
                                "counts": {
                                    "visits": len(ptr.usdm.get("visits", ptr.usdm.get("encounters", []))),
                                    "activities": len(ptr.usdm.get("activities", [])),
                                    "sais": len(ptr.usdm.get("scheduledActivityInstances", [])),
                                    "footnotes": len(ptr.usdm.get("footnotes", [])),
                                },
                            })
                    ...
                    mg_result = {
                        ...
                        "status": "completed" if schedule_timelines else "skipped",
                        "error": "No USDM data available" if not schedule_timelines else None,
                        "scheduleTimelines": schedule_timelines,
                        "counts": {
                            "timelines": len(schedule_timelines),
                            "visits": sum(t["counts"]["visits"] for t in schedule_timelines),
                            "activities": sum(t["counts"]["activities"] for t in schedule_timelines),
                            "sais": sum(t["counts"]["sais"] for t in schedule_timelines),
                            "footnotes": sum(t["counts"]["footnotes"] for t in schedule_timelines),
                        },
                    }
                    merge_group_results.append(mg_result)

                    # Collect USDMs for final output
                    for tl in schedule_timelines:
                        all_usdm.append({
                            "merge_group_id": mg.id,
                            "table_ids": [tl["sourceTableId"]],
                            "usdm": tl["usdm"],
                        })

                    logger.info(f"    {mg.id}: {len(schedule_timelines)} timelines assembled")

                # Store merge group results in the result object
                result.merge_group_results = merge_group_results

                # Build final output using timeline structure
                if all_usdm:
                    final_usdm = self._build_timeline_output(all_usdm, protocol_id)
    ...
    def _build_timeline_output(
        ...
        """Build timeline output from already-interpreted per-table USDMs.

        Unlike _merge_group_usdm (old), this does NOT deduplicate by name.
        Each table's USDM is kept as a separate timeline, and all entities
        are concatenated to avoid losing data from overlapping tables.
        """
        merged = {
            "protocolId": protocol_id,
            "visits": [],
            ...
            "footnotes": [],
            "scheduleTimelines": [],
            "_mergeGroups": list({g["merge_group_id"] for g in group_usdms}),
        }

        for group_data in group_usdms:
            usdm = group_data["usdm"]

            # Add as a timeline entry
            merged["scheduleTimelines"].append({
                "timelineId": f"ST-{group_data['merge_group_id']}",
                "sourceTableIds": group_data.get("table_ids", []),
                "usdm": usdm,
            })

            # Concatenate all entities (no dedup — keeps separate per-table data)
            merged["visits"].extend(usdm.get("visits", usdm.get("encounters", [])))
            merged["activities"].extend(usdm.get("activities", []))
            merged["scheduledActivityInstances"].extend(
                usdm.get("scheduledActivityInstances", [])
            )
            merged["footnotes"].extend(usdm.get("footnotes", []))
