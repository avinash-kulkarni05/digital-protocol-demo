...
class SequentialOrchestrator:
    ...
    async def run_extraction(
        ...
        try:
            ...
            # Filter to only requested modules (always include study_metadata)
            if modules:
                requested = set(modules)
                if "study_metadata" not in requested:
                    requested.add("study_metadata")
                pending_modules = [m for m in pending_modules if m in requested]
                # Update job total to reflect filtered count
                job = self.db.query(Job).filter(Job.id == job_id).first()
                if job:
                    job.total_modules = len(pending_modules)
                    self.db.commit()
    ...
    async def _extract_module(
        ...
        result, quality, from_cache = await self.extractor.extract_with_cache(
            ...
            model_name=settings.gemini_model or "gemini-3-pro-preview",
            use_cache=use_cache,
