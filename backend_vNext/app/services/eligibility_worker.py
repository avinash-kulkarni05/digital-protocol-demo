...
def _run_fully_automated(
    ...
    """
    Fully automated eligibility extraction — no human checkpoints.

    Runs section detection, then auto-confirms and runs full extraction.
    Used by "Extract All" mode.
    """
    import time

    logger = _setup_worker_logging(job_id, "auto")
    logger.info(f"Eligibility fully automated extraction started for job {job_id}")

    try:
        os.environ.setdefault('ELIGIBILITY_WORKER', 'true')

        from app.db import get_session_factory, EligibilityJob, Protocol
        from eligibility_analyzer.eligibility_section_detector import detect_eligibility_sections, DetectionResult
        from eligibility_analyzer.eligibility_extraction_pipeline import EligibilityExtractionPipeline
        ...
        logger.info(f"[Phase 1] Detected {len(sections)} section(s)")
        ...
        if not sections:
            logger.warning("No eligibility sections detected, marking as completed")
            _update_eligibility_job(job_id, {
                "status": "completed",
                "completed_at": datetime.utcnow(),
                "phase_progress": {"phase": "completed", "progress": 100},
        ...
        # --- Phase 2: Full Extraction (auto-confirm sections) ---
        logger.info("[Phase 2] Running full extraction (auto-confirmed sections)...")

        _update_eligibility_job(job_id, {
            "status": "extracting",
            "current_phase": "extraction",
            "confirmed_sections": detected_sections,
            "phase_progress": {"phase": "extraction", "progress": 0},
        }, logger)
        ...
        try:
            def on_progress(phase: str, stage: int, total_stages: int, stage_name: str):
                ...
                _update_eligibility_job(job_id, {
                    "current_phase": phase,
                    "phase_progress": {"phase": phase, "progress": progress, "stage": stage},
                }, logger)
            ...
            if not _update_eligibility_job(job_id, final_updates, logger):
                time.sleep(2)
                _update_eligibility_job(job_id, final_updates, logger)

            logger.info(f"Eligibility fully automated extraction completed for job {job_id}")

        finally:
            loop.close()

    except Exception as e:
        logger.error(f"Fully automated eligibility extraction failed: {e}", exc_info=True)
        _update_eligibility_job(job_id, {
            "status": "failed",
            "error_message": str(e)[:1000],
        }, logger)
...
def spawn_fully_automated_process(
    ...
    """
    Spawn fully automated eligibility extraction (no human checkpoints).
    ...
    process = ctx.Process(
        ...
        daemon=False,
        name=f"eligibility-auto-{str(job_id)[:8]}",
    )

    process.start()

    logging.getLogger(__name__).info(
        f"Spawned fully automated eligibility process {process.pid} for job {job_id}"
    )
