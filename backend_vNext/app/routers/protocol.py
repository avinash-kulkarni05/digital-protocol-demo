...
from app.db import get_db, Protocol, Job, FullExtractionJob, SOAJob, EligibilityJob
...
class FullExtractionResponse(BaseModel):
    """Response model for full extraction (all 3 pipelines)."""
    full_job_id: str
    protocol_id: str
    status: str
    message: str
...
class FullExtractionStatusResponse(BaseModel):
    """Response model for full extraction status."""
    full_job_id: str
    protocol_id: str
    status: str
    overall_progress: int
    main_job_id: Optional[str] = None
    soa_job_id: Optional[str] = None
    eligibility_job_id: Optional[str] = None
    main_status: str = "pending"
    soa_status: str = "pending"
    eligibility_status: str = "pending"
    main_progress: int = 0
    soa_progress: int = 0
    eligibility_progress: int = 0
...
class ModuleInfo(BaseModel):
    """Response model for a single extraction module."""
    module_id: str
    display_name: str
    wave: int
    priority: int
...
async def list_protocols(
    ...
    return [
        ProtocolListResponse(
            ...
            studyTitle=protocol.protocol_name or protocol.filename.replace(".pdf", "").replace("_", " "),
            studyName=protocol.protocol_name,
            sponsor=protocol.sponsor,
            nctNumber=protocol.nct_number,
...
async def list_modules():
    """
    List all available extraction modules with their metadata.

    Returns the list of 17 modules grouped by wave, for the
    Custom Extract modal.
    """
    from app.module_registry import EXTRACTION_MODULES

    modules = []
    for module_id, config in EXTRACTION_MODULES.items():
    ...
    # Sort by wave, then priority, then order
    modules.sort(key=lambda m: (m.wave, m.priority))
    return modules
...
async def upload_protocol(
    ...
    nct_number: Optional[str] = Form(None),
    user_email: Optional[str] = Form(None),
    db: Session = Depends(get_db),
):
    """
    Upload a protocol PDF file with optional study metadata.

    The file binary data is stored in the database (file_data column).
    Duplicate files (same hash) return the existing record.
    Accepts optional form fields: study_name, sponsor, nct_number, user_email.
    """
    ...
    if existing:
        ...
        if existing.file_data is None:
            ...
            needs_update = True
        # Always overwrite metadata when user provides new values
        if study_name:
            existing.protocol_name = study_name
            needs_update = True
        elif existing.protocol_name is None:
        ...
        if sponsor:
            ...
            needs_update = True
        if nct_number:
            existing.nct_number = nct_number
            needs_update = True
        if user_email:
        ...
        return ProtocolResponse(
            ...
            studyName=existing.protocol_name,
            sponsor=existing.sponsor,
            nctNumber=existing.nct_number,
            userEmail=existing.user_email,
            gemini_file_uri=existing.gemini_file_uri,
            created_at=existing.created_at.isoformat(),
        )

    # Use study_name if provided, otherwise derive from filename
    protocol_name = study_name or (file.filename.rsplit('.', 1)[0] if '.' in file.filename else file.filename)

    # Create new protocol record with binary data and metadata
    protocol = Protocol(
...
async def start_extraction(
    ...
    # Check if PDF data is available in database
    if not protocol.file_data:
        raise HTTPException(status_code=400, detail="Protocol PDF data not found")

    # Create a temp file for extraction pipeline
    import tempfile
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
        tmp.write(protocol.file_data)
        pdf_path = tmp.name
    logger.info(f"Created temp file for protocol {protocol_id}: {pdf_path}")
...
@router.post("/{protocol_id}/extract-all", response_model=FullExtractionResponse)
async def start_full_extraction(
    protocol_id: UUID,
    db: Session = Depends(get_db),
):
    """
    Start full extraction: main modules + SOA + eligibility in parallel.

    All 3 pipelines run without human checkpoints (auto-confirm mode).
    Returns a full_job_id that tracks all 3 sub-pipelines.
    """
    # Get protocol
    protocol = db.query(Protocol).filter(Protocol.id == protocol_id).first()
    if not protocol:
    ...
    # Create FullExtractionJob record
    import uuid as uuid_module
    full_job = FullExtractionJob(
    ...
    from app.services.full_extraction_worker import spawn_full_extraction_process
    ...
    logger.info(
        f"Started full extraction job {full_job.id} in process {process.pid} "
        f"for protocol {protocol_id}"
    )

    return FullExtractionResponse(
        full_job_id=str(full_job.id),
        protocol_id=str(protocol_id),
        status="running",
        message=f"Full extraction started (PID: {process.pid}). All 3 pipelines running in parallel.",
    )


@router.get("/{protocol_id}/extract-all/status", response_model=FullExtractionStatusResponse)
async def get_full_extraction_status(
    protocol_id: UUID,
    db: Session = Depends(get_db),
):
    """
    Get status of the latest full extraction job for a protocol.

    Returns progress of all 3 sub-pipelines.
    """
    full_job = db.query(FullExtractionJob).filter(
        FullExtractionJob.protocol_id == protocol_id
    ).order_by(FullExtractionJob.created_at.desc()).first()

    if not full_job:
        raise HTTPException(status_code=404, detail="No full extraction job found for this protocol")

    return FullExtractionStatusResponse(
        full_job_id=str(full_job.id),
        protocol_id=str(full_job.protocol_id),
        status=full_job.status,
        overall_progress=full_job.overall_progress or 0,
        main_job_id=str(full_job.main_job_id) if full_job.main_job_id else None,
        soa_job_id=str(full_job.soa_job_id) if full_job.soa_job_id else None,
        eligibility_job_id=str(full_job.eligibility_job_id) if full_job.eligibility_job_id else None,
        main_status=full_job.main_status or "pending",
        soa_status=full_job.soa_status or "pending",
        eligibility_status=full_job.eligibility_status or "pending",
        main_progress=full_job.main_progress or 0,
        soa_progress=full_job.soa_progress or 0,
        eligibility_progress=full_job.eligibility_progress or 0,
        error_message=full_job.error_message,
        created_at=full_job.created_at.isoformat() if full_job.created_at else None,
        updated_at=full_job.updated_at.isoformat() if full_job.updated_at else None,
    )
