...
class SOAJob(Base):
    ...
    # Status: detecting_pages, awaiting_page_confirmation, extracting, interpreting_tables, analyzing_merges, awaiting_merge_confirmation, classifying, awaiting_classification_review, assembling, interpreting, validating, completed, failed
    ...
    # Classification Review (Phase 3.6) - stores classifier results for human review
    classification_review = Column(JSONB, nullable=True)
...
class SOATableResult(Base):
    ...
    raw_usdm_data = Column(JSONB, nullable=True)  # Pre-interpretation USDM (snapshot before interpretation overwrites usdm_data)
...
class FullExtractionJob(Base):
    """Full extraction job tracking all 3 pipelines (main + SOA + eligibility) as one job."""

    __tablename__ = "full_extraction_jobs"
    __table_args__ = {"schema": SCHEMA_NAME}
    ...
    main_job_id = Column(UUID(as_uuid=True), nullable=True)  # Links to jobs table
    soa_job_id = Column(UUID(as_uuid=True), nullable=True)   # Links to soa_jobs table
    eligibility_job_id = Column(UUID(as_uuid=True), nullable=True)  # Links to eligibility_jobs table

    # Sub-pipeline statuses
    main_status = Column(String(50), default="pending")      # pending, running, completed, failed
    soa_status = Column(String(50), default="pending")
    eligibility_status = Column(String(50), default="pending")

    # Sub-pipeline progress (0-100)
    main_progress = Column(Integer, default=0)
    soa_progress = Column(Integer, default=0)
    eligibility_progress = Column(Integer, default=0)
...
Index("idx_full_extraction_jobs_protocol_id", FullExtractionJob.protocol_id)
Index("idx_full_extraction_jobs_status", FullExtractionJob.status)
