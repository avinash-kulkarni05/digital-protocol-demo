...
class InterpretationPipeline:
    ...
    async def run(
        ...
        # Populate OMOP concept count from Stage 5 results
        omop_mappings = accumulated_data.get("omop_mappings")
        if omop_mappings and isinstance(omop_mappings, dict):
            result.omop_concepts_count = len(omop_mappings.get("mappings", []))
    ...
    async def _execute_stage(
        ...
        if stage_num == 1:
            ...
            # Use RAG-based mapping (curated + semantic + LLM validation)
            # Falls back to legacy ATHENA pattern matching if RAG unavailable
            mappings = await self._map_to_omop_with_rag(
    ...
    async def _map_to_omop_with_rag(
        self,
        term_extractions: Dict[str, Any],
        athena_db_path: Optional[str]
    ) -> Dict[str, Any]:
        """
        Stage 5: Map terms to OMOP concepts using RAG approach (PARALLELIZED).

        Three-tier mapping:
        1. Curated mappings (demographics, common terms) - 100% confidence
        2. Semantic vector search (ChromaDB) - variable confidence
        3. LLM validation (for ambiguous matches) - validated confidence

        Falls back to legacy ATHENA pattern matching if RAG is unavailable.

        OPTIMIZATION: Uses asyncio.gather with semaphore for parallel mapping,
        controlled by OMOP_PARALLEL_WORKERS (default 10).
        """
        try:
            from eligibility_analyzer.omop_rag import get_rag_mapper
            rag_mapper = get_rag_mapper()
        except Exception as e:
            logger.warning(f"RAG mapper not available, falling back to legacy: {e}")
            return await self._map_to_omop(term_extractions, athena_db_path)

        mappings = {"mappings": [], "unmapped": [], "statistics": {}}

        # Domain hint normalization map
        domain_mapping = {
            "Condition": "Condition",
            "Drug": "Drug",
            "Measurement": "Measurement",
        ...
        if term_extractions:
            for extraction in term_extractions.get("extractions", []):
                ...
                for term in extraction.get("terms", []):
                    ...
                    domain_hint = llm_exp.get("omopDomainHint")
                    ...
                    terms_to_process.append({
                        ...
                        "llmDomainHint": domain_hint,
                        "mappedDomain": domain_mapping.get(domain_hint) if domain_hint else None,
                    })
        ...
        logger.info(f"Stage 5 (RAG): Processing {total_terms} terms via batch mapping")

        # Build batch input for RAGMapper.map_terms_batch()
        batch_input = [
            {
                "term": t["primaryTerm"],
                "domain_hint": t.get("mappedDomain"),
                "criterion_id": t["criterionId"],
                "atomic_id": t["atomicId"],
            }
            for t in terms_to_process
        ]

        # Single batch call with batched embeddings + concurrent pgvector
        batch_results = await rag_mapper.map_terms_batch(
            batch_input,
            max_concurrent=OMOP_PARALLEL_WORKERS,
        ...
        for i, result in enumerate(batch_results):
            ...
            if result.is_mapped:
                ...
                mappings["unmapped"].append({
                    ...
                    "atomicId": term_info["atomicId"],
                    "term": term_info["primaryTerm"],
                    "domain": term_info.get("mappedDomain") or "Unknown",
        ...
        mappings["statistics"] = {
            ...
            "llmValidatedCount": llm_validated_count,
            "ragEnabled": True,
        }

        # Get RAG mapper stats
        rag_stats = rag_mapper.get_stats()
        mappings["statistics"]["ragStats"] = rag_stats

        logger.info(
            f"Stage 5 (RAG) complete: {mapped_count}/{total_terms} mapped ({mappings['statistics']['mappingRate']:.0%}) "
            f"[curated={curated_count}, semantic={semantic_count}, llm={llm_validated_count}]"
        )

        # Stage 5.5: Apply clinical reasoning to RAG-unmapped terms
        if unmapped_count > 0:
            db_path = athena_db_path or os.environ.get('ATHENA_DB_PATH')
            if db_path and Path(db_path).exists():
                logger.info(f"Stage 5.5 (RAG): Applying clinical reasoning to {unmapped_count} unmapped terms")
                try:
                    ...
                    logger.info(
                        f"Stage 5.5 (RAG) complete: {len(stage_5_5_result.get('newly_mapped', []))} newly mapped "
                        f"from clinical reasoning, final rate: {mappings['statistics']['mappingRate']:.0%}"
                    )
                except Exception as e:
                    logger.error(f"Stage 5.5 (RAG) clinical reasoning failed: {e}")
                    mappings["statistics"]["stage_5_5_error"] = str(e)
            else:
                logger.info("Stage 5.5 (RAG): Skipped - ATHENA database not available for clinical reasoning")
    ...
    def _generate_sql_templates(
        ...
        if omop_mappings:
            for mapping in omop_mappings.get("mappings", []):
                ...
                # Handle RAG mapper format (singular conceptId)
                concept_id = mapping.get("conceptId")
                if concept_id:
                    concept_lookup[key] = {
                        "concept_ids": [concept_id],
                        "domain": mapping.get("domainId", "Observation"),
                        "concept_name": mapping.get("conceptName"),
                        "vocabulary_id": mapping.get("vocabularyId"),
                    }
                else:
                    # Fallback: Handle legacy format (concepts array)
...
{full_sql}
    ...
    def _generate_atomic_sql(
        ...
        # Special handling for demographic criteria (gender, age)
        # When concept_ids is empty, check if this is a gender criterion and populate
        if not concept_ids and omop_table == "observation":
            is_gender, detected_gender = self._is_gender_criterion(atomic_text)
            if is_gender and detected_gender:
                gender_concept_id = self._get_gender_concept_id(detected_gender)
                concept_ids = [gender_concept_id]
                concept_name = detected_gender.upper()  # "MALE" or "FEMALE"
        ...
        return {
            ...
            "conceptNames": [concept_name] if concept_name else [],
            "sql": sql,
...
-- Requires manual SQL generation"""
    ...
    def _generate_atomic_sql_from_tree(
        ...
        # Special handling for gender criteria when concept_ids is empty
        if not concept_ids and omop_table == "observation":
            is_gender, detected_gender = self._is_gender_criterion(atomic_text)
            if is_gender and detected_gender:
                gender_concept_id = self._get_gender_concept_id(detected_gender)
