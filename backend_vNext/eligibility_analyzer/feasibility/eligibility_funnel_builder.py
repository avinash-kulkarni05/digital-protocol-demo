...
class EligibilityFunnelBuilder:
    ...
    async def _validate_and_update_omop_mappings_llm(
        ...
        for atomic in atomics:
            ...
            for m in candidate_mappings:
                ...
                singular_concept_id = m.get("conceptId")
                if singular_concept_id:
                    concept_dict = {
                        "concept_id": singular_concept_id,
                        "concept_name": m.get("conceptName", ""),
                        "concept_code": m.get("conceptCode", ""),
                        "vocabulary_id": m.get("vocabularyId", ""),
                        "domain_id": m.get("domainId", ""),
                    }
                    if concept_dict not in all_concepts:
    ...
    def _create_atomic_from_node(
        ...
        if omop_mapping:
            # Handle RAG mapper format (singular conceptId)
            singular_concept_id = omop_mapping.get("conceptId")
            if singular_concept_id:
                concept_ids.append(singular_concept_id)
                concept_names.append(omop_mapping.get("conceptName", ""))
                concept_codes.append(omop_mapping.get("conceptCode", ""))
                vocab = omop_mapping.get("vocabularyId", "")
