...
function FootnotesPanel({ footnotes, onViewSource, onFieldUpdate, tableIndex = 0, onAddFootnote, currentPage = 1 }: FootnotesPanelProps) {
    ...
    async function startSOAExtraction() {
      ...
      try {
        ...
        if (latestJob.job_id && latestJob.status === 'completed' && latestJob.has_results) {
          // Load completed results - always show extraction view on initial load
          // User can click "Complete Review" to see interpretation results
          ...
          // Stay in extraction view - viewMode defaults to 'extraction'
          return;
        }
    ...
    try {
      ...
      if (status.status === 'awaiting_page_confirmation' && status.detected_pages) {
        ...
        try {
          ...
          // Check if interpretation ran using interpretation-stages endpoint
          try {
            const stagesResponse = await api.soa.getInterpretationStages(jobId);
            if (stagesResponse.groups && stagesResponse.groups.length > 0) {
              console.log('[SOA] Interpretation complete, switching to interpretation view');
              setViewMode('interpretation');
            }
          } catch (stagesErr) {
            // No interpretation stages - stay in extraction view
            console.log('[SOA] No interpretation stages available');
          }
        } catch (fetchErr) {
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        if (status.status === 'awaiting_merge_confirmation') {
          ...
          // Check if interpretation ran using interpretation-stages endpoint
          try {
            const stagesResponse = await api.soa.getInterpretationStages(soaJobId);
            if (stagesResponse.groups && stagesResponse.groups.length > 0) {
              console.log('[SOA] Interpretation complete, switching to interpretation view');
              setViewMode('interpretation');
            }
