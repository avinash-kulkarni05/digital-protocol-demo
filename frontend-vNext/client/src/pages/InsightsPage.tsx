...
interface Stage8Decision {
  ...
  isOpenEnded?: boolean;
  protocolContext?: Record<string, unknown>;
}
...
function CycleExpansionTab({ data }: { data: Stage8Data | null }) {
  ...
  const metrics = data.metrics || {} as Stage8Data['metrics'];
    ...
    <div className="space-y-6" data-testid="tab-content-cycles">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="p-3 text-center" data-testid="metric-review-items-count">
          <p className="text-xl font-bold text-gray-600">{metrics.reviewItemsCount ?? 0}</p>
          <p className="text-xs text-muted-foreground">Review Items</p>
        </Card>
        <Card className="p-3 text-center" data-testid="metric-expansion-rate">
          <p className="text-xl font-bold text-gray-700">{Math.round((metrics.expansionRate ?? 0) * 100)}%</p>
        ...
        <Card className="p-3 text-center" data-testid="metric-validation-flags">
          <p className="text-xl font-bold text-gray-700">{metrics.validationFlags ?? 0}</p>
            ...
            {needsReview.map(([encId, decision]) => (
              <div key={encId} className="p-4 bg-white/70 rounded-lg border border-gray-300">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-xs bg-gray-100 border-gray-300 text-gray-700">
                      {(decision.patternType || 'unknown').replace(/_/g, ' ')}
                    </Badge>
                ...
                <div className="flex items-center gap-2 mt-3 flex-wrap">
                  <Badge variant="secondary" className="text-xs">
                    Cycle Length: {decision.cycleLengthDays ?? 'N/A'} days
                  </Badge>
              ...
              <Collapsible key={exp.id} open={expanded.has(exp.id)} onOpenChange={() => toggleExpanded(exp.id)}>
                ...
                <CollapsibleContent>
                  <div className="px-3 pb-3 pt-2 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        ...
                        <div className="space-y-1 text-sm">
                          <div><span className="text-gray-600">Pattern:</span> <span className="text-gray-800 font-mono">{exp.originalRecurrence?.pattern || exp.originalRecurrence?.type || 'N/A'}</span></div>
                          <div><span className="text-gray-600">Type:</span> <span className="text-gray-800">{(exp.originalRecurrence?.type || 'unknown').replace(/_/g, ' ')}</span></div>
                          ...
                          {exp.originalRecurrence?.maxCycles && (
                            <div><span className="text-gray-600">Max Cycles:</span> <span className="text-gray-800">{exp.originalRecurrence.maxCycles}</span></div>
                          )}
                    ...
                    {exp.expandedEncounterIds && exp.expandedEncounterIds.length > 0 && (
                    ...
                    {exp.expandedSaiIds && exp.expandedSaiIds.length > 0 && (
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {exp.expandedCycleNumbers && exp.expandedCycleNumbers.length > 0 && (
                  ...
                  <div className="px-3 pb-3 pt-2 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                        ...
                        <div className="space-y-1 text-sm">
                          <div><span className="text-gray-600">Type:</span> <span className="text-gray-800">{(decision.patternType || 'unknown').replace(/_/g, ' ')}</span></div>
                          <div><span className="text-gray-600">Cycle Length:</span> <span className="text-gray-800 font-bold">{decision.cycleLengthDays ?? 'N/A'} days</span></div>
