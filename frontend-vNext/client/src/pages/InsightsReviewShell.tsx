  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {data.mappings.length === 0 && (
        <EmptyStageState icon={Layers} label="domain mappings" description="No activities were categorized into CDISC domains for this protocol." />
      )}
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {data.expansions.length === 0 && (
        <EmptyStageState icon={GitBranch} label="activity expansions" description="No activities required component decomposition for this protocol." />
      )}
...
function AlternativesPanel({ data, onViewSource }: { data: Stage4Data | null; onViewSource: (page: number) => void }) {
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {data.expansions.length === 0 && (
        <EmptyStageState icon={GitBranch} label="alternatives" description="No alternative activity resolutions were identified for this protocol." />
      )}
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {data.enrichments.length === 0 && (
        <EmptyStageState icon={TestTube} label="specimens" description="No biospecimen collection activities were identified for this protocol." />
      )}
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {data.conditions.length === 0 && (
        <EmptyStageState icon={FileText} label="conditions" description="No footnote-based conditions were identified for this protocol." />
      )}
...
function CyclesPanel({ data, onViewSource }: { data: Stage8Data | null; onViewSource: (page: number) => void }) {
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {(!data.expansions || data.expansions.length === 0) && Object.keys(data.decisions || {}).length === 0 && (
        <EmptyStageState icon={Repeat} label="cycles" description="No cycle expansion patterns were detected for this protocol." />
      )}
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 rounded-xl bg-gray-50 border">
                        ...
                        <p className="text-sm font-mono">{exp.originalRecurrence?.pattern || exp.originalRecurrence?.type || 'N/A'}</p>
                        {(exp.originalRecurrence?.startCycle || exp.originalRecurrence?.cycleDay) && (
                          <p className="text-xs text-muted-foreground mt-1">
                            {exp.originalRecurrence?.startCycle
                              ? `Cycles ${exp.originalRecurrence.startCycle} - ${exp.originalRecurrence.endCycle || '?'}`
                              : `Cycle Day ${exp.originalRecurrence.cycleDay}${exp.originalRecurrence.maxCycles ? `, Max ${exp.originalRecurrence.maxCycles} cycles` : ''}`
                            }
                          </p>
                        )}
                      </div>
      ...
      {data.decisions && Object.keys(data.decisions).length > 0 && (
        <InsightCard
          title="AI Decisions"
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      {Object.keys(data.decisions).length === 0 && (
        <EmptyStageState icon={Microscope} label="protocol mining results" description="No module matching decisions were produced for this protocol." />
      )}

      {decisions.length > 0 && <InsightCard
...
function EmptyStageState({ icon: Icon, label, description }: { icon: React.ElementType; label: string; description: string }) {
  return (
    <div className="flex items-center justify-center h-48">
      <div className="text-center">
        <Icon className="w-10 h-10 text-gray-300 mx-auto mb-3" />
        <p className="text-sm font-medium text-gray-500">No {label.toLowerCase()} found</p>
...
export default function InsightsReviewShell(props?: { onBack?: () => void; initialGroupId?: string | null } & Record<string, any>) {
  const onBack = props?.onBack;
  const initialGroupId = props?.initialGroupId || null;
  ...
  // Per-table/group selection for insights
  const [allGroups, setAllGroups] = useState<any[]>([]);
  const [selectedGroupIndex, setSelectedGroupIndex] = useState(0);
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(groupIdParam || initialGroupId);
  ...
  //   const groupData = groups.find((g: any) => g.merge_group_id === groupId);
  //   if (groupData) {
  //     const stageResults = groupData.stage_results || {};
  //     setStage1Data(stageResults['1'] ? stageResults['1'] as Stage1Data : null);
  //     setStage2Data(stageResults['2'] ? stageResults['2'] as Stage2Data : null);
  //     setStage4Data(stageResults['4'] ? stageResults['4'] as Stage4Data : null);
  //     setStage5Data(stageResults['5'] ? stageResults['5'] as Stage5Data : null);
  //     setStage6Data(stageResults['6'] ? stageResults['6'] as Stage6Data : null);
  //     setStage8Data(stageResults['8'] ? stageResults['8'] as Stage8Data : null);
  //     setStage9Data(stageResults['9'] ? stageResults['9'] as Stage9Data : null);
  //   }
  // };

  const [dataSource, setDataSource] = useState<'database' | 'static' | null>(null);

  // Helper to apply stage results from a group to the individual stage states
  const _applyStageResults = useCallback((stageResults: Record<string, any>) => {
    setStage1Data(stageResults['1'] as Stage1Data || null);
    setStage2Data(stageResults['2'] as Stage2Data || null);
    setStage4Data(stageResults['4'] as Stage4Data || null);
    setStage5Data(stageResults['5'] as Stage5Data || null);
    setStage6Data(stageResults['6'] as Stage6Data || null);
    setStage8Data(stageResults['8'] as Stage8Data || null);
    setStage9Data(stageResults['9'] as Stage9Data || null);
  }, []);

  // When user switches groups, load that group's stage data
  const handleGroupChange = useCallback((index: number) => {
    setSelectedGroupIndex(index);
    setSelectedGroupId(allGroups[index]?.merge_group_id || null);
    if (allGroups[index]) {
      const stageResults = allGroups[index].stage_results || {};
      _applyStageResults(stageResults);
      console.log('[InsightsReviewShell] Switched to group:', allGroups[index].merge_group_id);
    }
  }, [allGroups, _applyStageResults]);
      ...
      try {
        ...
        if (latestJob?.job_id) {
          ...
          if (stagesResponse?.groups && stagesResponse.groups.length > 0) {
            // Store all groups for the group selector
            setAllGroups(stagesResponse.groups);

            // Use the group from URL param / initialGroupId, or fall back to first group
            const targetGroupId = selectedGroupId || stagesResponse.groups[0].merge_group_id;
            if (!selectedGroupId) setSelectedGroupId(targetGroupId);
            const groupData = stagesResponse.groups.find(
              (g: any) => g.merge_group_id === targetGroupId ||
                g.source_table_ids?.includes(targetGroupId)
            ) || stagesResponse.groups[0];
            const targetIndex = stagesResponse.groups.indexOf(groupData);
            setSelectedGroupIndex(targetIndex >= 0 ? targetIndex : 0);
            // Normalize selectedGroupId to the actual merge_group_id
            setSelectedGroupId(groupData.merge_group_id);

            // Load stage data from the matched group
            const stageResults = groupData.stage_results || {};
            _applyStageResults(stageResults);

            setDataSource('database');
            console.log('[InsightsReviewShell] Loaded stage data from database,', stagesResponse.groups.length, 'groups');
            return;
          }
        }
      } catch (err) {
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      <header className="shrink-0 border-b bg-white/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => onBack ? onBack() : navigate(`/soa-analysis?studyId=${studyId}`)} className="rounded-xl" data-testid="button-back">
              ...
              <div className="flex gap-2">
                {groups.map((group: any) => (
                  <Button
                    ...
                    variant={selectedGroupId === group.merge_group_id ? 'default' : 'outline'}
                    size="sm"
                    onClick={() => handleGroupChange(group.merge_group_id)}
                  >
      ...
      {/* Table/Group Selector - shown when multiple groups exist */}
      {allGroups.length > 1 && (
        <div className="flex items-center gap-2 px-6 py-3 bg-gray-50 border-b border-gray-200">
          <span className="text-sm font-medium text-gray-600 mr-2">Table:</span>
          {allGroups.map((group, idx) => (
            <button
              key={group.merge_group_id || idx}
            ...
            >
              {group.merge_group_id || `Group ${idx + 1}`}
              {group.source_table_ids?.length > 0 && (
