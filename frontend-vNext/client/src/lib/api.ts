...
export const api = {
  ...
  extraction: {
    startExtraction: async (protocolId: string, modules?: string[]): Promise<{ job_id: string; protocol_id: string; status: string; message: string }> => {
      const response = await fetch(`${BACKEND_API_BASE}/protocols/${encodeURIComponent(protocolId)}/extract`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ resume: true, modules: modules || null }),
      });
      if (!response.ok) {
    ...
    startFullExtraction: async (protocolId: string): Promise<FullExtractionResponse> => {
      const response = await fetch(`${BACKEND_API_BASE}/protocols/${encodeURIComponent(protocolId)}/extract-all`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) {
        throw new Error(`Failed to start full extraction: ${response.statusText}`);
      }
      return response.json();
    },

    getFullExtractionStatus: async (protocolId: string): Promise<FullExtractionStatus> => {
      const response = await fetch(`${BACKEND_API_BASE}/protocols/${encodeURIComponent(protocolId)}/extract-all/status`);
      if (!response.ok) {
        throw new Error(`Failed to get full extraction status: ${response.statusText}`);
      }
      return response.json();
    },

    getModules: async (): Promise<ModuleInfo[]> => {
      const response = await fetch(`${BACKEND_API_BASE}/protocols/modules`);
      if (!response.ok) {
  ...
  soa: {
    ...
    // Confirm extraction results and start interpretation (Phase 3b)
    confirmExtraction: async (jobId: string): Promise<{ job_id: string; status: string; message: string }> => {
      const response = await fetch(`${BACKEND_API_BASE}/soa/jobs/${jobId}/confirm-extraction`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || `Failed to confirm extraction: ${response.statusText}`);
      }
      return response.json();
    },
    ...
    // Get classification review data
    getClassificationReview: async (jobId: string): Promise<ClassificationReviewData> => {
      const response = await fetch(`${BACKEND_API_BASE}/soa/jobs/${jobId}/classification-review`);
      if (!response.ok) {
        throw new Error(`Failed to get classification review: ${response.statusText}`);
      }
      return response.json();
    },

    // Confirm classification profiles and start interpretation
    confirmClassification: async (jobId: string, confirmedProfiles: Record<string, ClassificationProfile>): Promise<{ soa_job_id: string; status: string; message: string }> => {
      const response = await fetch(`${BACKEND_API_BASE}/soa/jobs/${jobId}/classification/confirm`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || `Failed to confirm classification: ${response.statusText}`);
      }
      return response.json();
    },
  },
  ...
  observations: {
    create: async (data: { name: string; description: string; date: string; files?: File[] }): Promise<{ success: boolean; message: string }> => {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `Failed to save observation: ${response.statusText}`);
      }
      return response.json();
    },
  },
...
export interface SOAJobStatus {
  job_id: string;
  protocol_id: string;
  status: 'detecting_pages' | 'awaiting_page_confirmation' | 'extracting' | 'awaiting_extraction_review' | 'analyzing_merges' | 'awaiting_merge_confirmation' | 'classifying' | 'awaiting_classification_review' | 'interpreting' | 'validating' | 'completed' | 'failed';
  ...
  classification_review?: ClassificationReviewData;
...
export interface ClassificationProfile {
  tableStructureType: string;
  confidence: number;
  characteristics: string[];
  stageGuidance: {
    stage7_timing?: string;
    stage8_cycles?: string;
    stage9_mining?: string;
    general?: string;
    [key: string]: string | undefined;
  };
  structuralFeatures: Record<string, any>;
  userModified?: boolean;
}

export interface ClassificationReviewData {
  status: 'pending_confirmation' | 'confirmed';
  classifiedAt: string;
  groups: Record<string, ClassificationProfile>;
}
...
export interface SOATableResult {
  ...
  usdm_data?: any;
  raw_usdm_data?: any;
  interpretation_stages?: Record<string, any>;
}
...
// Full Extraction Types (Extract All mode)
...
export interface FullExtractionStatus {
  full_job_id: string;
  protocol_id: string;
  ...
  main_job_id?: string;
  soa_job_id?: string;
  eligibility_job_id?: string;
  main_status: string;
  soa_status: string;
  eligibility_status: string;
  main_progress: number;
  soa_progress: number;
  eligibility_progress: number;
  error_message?: string;
  created_at?: string;
  updated_at?: string;
}

// Module Info (for Custom Extract modal)
...
export interface MergeGroup {
  ...
  // Full decision path from merge analyzer
  decisionPath?: Array<{
    level: number;
    name: string;
    decision: string;
