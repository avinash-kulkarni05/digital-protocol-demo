...
export const CLINICAL_CATEGORIES: Record<string, { icon: React.ElementType; color: string; bgColor: string; borderColor: string }> = {
  "Demographics": { icon: Users, color: "text-blue-600", bgColor: "bg-blue-50", borderColor: "border-blue-200" },
  "Disease Characteristics": { icon: Activity, color: "text-purple-600", bgColor: "bg-purple-50", borderColor: "border-purple-200" },
  "Disease Status": { icon: Activity, color: "text-purple-600", bgColor: "bg-purple-50", borderColor: "border-purple-200" },
  "Prior Therapy": { icon: Pill, color: "text-orange-600", bgColor: "bg-orange-50", borderColor: "border-orange-200" },
  "Prior/Concurrent Therapy": { icon: Pill, color: "text-orange-600", bgColor: "bg-orange-50", borderColor: "border-orange-200" },
  "Organ Function": { icon: Heart, color: "text-red-600", bgColor: "bg-red-50", borderColor: "border-red-200" },
  "Laboratory": { icon: TestTube, color: "text-cyan-600", bgColor: "bg-cyan-50", borderColor: "border-cyan-200" },
  "Informed Consent": { icon: FileCheck, color: "text-gray-600", bgColor: "bg-gray-100", borderColor: "border-gray-300" },
  "Reproductive Status": { icon: Shield, color: "text-pink-600", bgColor: "bg-pink-50", borderColor: "border-pink-200" },
  "Washout Period": { icon: Clock, color: "text-amber-600", bgColor: "bg-amber-50", borderColor: "border-amber-200" },
...
export function CriterionCard({
  ...
  const statusConfig = {
    "fully_queryable": { color: "bg-emerald-500", label: "Fully Queryable", icon: CheckCircle2 },
    "partially_queryable": { color: "bg-amber-500", label: "Needs Mapping", icon: AlertCircle },
    "requires_manual": { color: "bg-gray-400", label: "Manual Review", icon: Circle },
  };
  ...
  const typeColors = {
    inclusion: {
      band: "bg-gradient-to-b from-green-400 to-green-600",
      badge: "bg-green-50 border-green-200 text-green-700",
      indexBg: "bg-green-50 border-green-200 text-green-600",
    },
    exclusion: {
      band: "bg-gradient-to-b from-red-400 to-red-600",
      badge: "bg-red-50 border-red-200 text-red-700",
      indexBg: "bg-red-50 border-red-200 text-red-500",
    },
  };
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              {qeb.provenance?.pageNumber && onViewProvenance && (
                <button
                  ...
                  className="ml-auto flex items-center gap-1 text-[9px] text-blue-600 hover:text-blue-800 transition-colors"
                >
            ...
            {isLongText && (
              <button
                ...
                className="mt-1 flex items-center gap-1 text-[11px] text-blue-600 hover:text-blue-800 transition-colors"
              >
                {isExpanded ? (
                  <>
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-2">
              ...
              {qeb.isKillerCriterion && (
                <span className="inline-flex items-center gap-0.5 text-[9px] px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200 font-medium">
              ...
              {/* Sub-criteria Count */}
              {qeb.atomicCount > 1 && (
                <span className="inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 border border-purple-200 font-medium">
              ...
              {qeb.internalLogic === "COMPLEX" && (
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200 font-medium">
                  Complex Logic
                </span>
              )}
            </div>
            ...
            {qeb.omopConcepts && qeb.omopConcepts.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {qeb.omopConcepts.slice(0, 4).map((concept, cIdx) => (
                  <span
                    key={cIdx}
                    className="inline-flex items-center text-[9px] px-2 py-0.5 rounded-md bg-slate-50 text-slate-600 border border-slate-200"
                ...
                {qeb.omopConcepts.length > 4 && (
                  <span className="text-[9px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 font-medium">
