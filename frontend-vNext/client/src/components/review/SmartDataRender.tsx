...
function RenderValue({ 
  ...
  // Detect {unit, value} measurement objects and render as "value unit"
  if (typeof value === 'object' && value !== null && !Array.isArray(value) &&
      'unit' in value && 'value' in value && Object.keys(value).length <= 3) {
    const display = `${(value as any).value ?? ''}${(value as any).unit ? ' ' + (value as any).unit : ''}`.trim();
    return <span className="text-gray-700">{display || 'Not specified'}</span>;
  }
