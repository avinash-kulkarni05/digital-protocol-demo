        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-3 mb-3">
              <h4 className="font-bold text-blue-900 text-lg">EDC Specifications</h4>
            ...
            {edc.vendor_name && (
              <div className="bg-white/70 rounded-lg p-3 border border-blue-200 mb-3">
                <span className="text-xs font-medium text-blue-800 uppercase tracking-wider block mb-1">Vendor</span>
                <EditableText
            ...
            {edc.system_capabilities?.length > 0 && (
              <div className="mb-3">
                <span className="text-xs font-medium text-blue-800 uppercase tracking-wider block mb-2">System Capabilities</span>
                <div className="flex flex-wrap gap-2">
                  {edc.system_capabilities.map((cap: string, idx: number) => (
                    <span key={idx} className="text-xs bg-white/70 text-blue-700 px-3 py-1.5 rounded-full border border-blue-200 font-medium">
                      {cap.replace(/_/g, " ")}
                    </span>
                  ))}
                </div>
            ...
            {languages.length > 0 && (
              <div>
                <span className="text-xs font-medium text-blue-800 uppercase tracking-wider block mb-2">Language Requirements</span>
                <div className="flex flex-wrap gap-2">
                  {languages.map((lang: string, idx: number) => (
                    <span key={idx} className="inline-flex items-center gap-1 text-xs bg-blue-100 text-blue-800 px-3 py-1.5 rounded-full font-medium">
                  ...
                  <tr key={integration.id || idx} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <EditableText
                        ...
                        className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-indigo-100 text-indigo-700 rounded"
                      />
                    </td>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {module.is_repeating && (
                          <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded">Repeating</span>
                        )}
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-1">
                              {module.visit_schedule.map((visit: string, vIdx: number) => (
                                <span key={vIdx} className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded">
                                  {visit}
                                </span>
                              ))}
                            </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-violet-50 border border-purple-200 rounded-2xl p-5">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-purple-900 text-lg">SDV Strategy</h4>
                <p className="text-sm text-purple-700">Source Data Verification approach</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {sdvStrategy.approach && (
              <div className="bg-white rounded-lg p-3 border border-purple-200">
                <span className="text-xs font-medium text-purple-700 uppercase block mb-1">Approach</span>
                <EditableText
            ...
            {sdvStrategy.sdv_percentage !== undefined && (
              <div className="bg-white rounded-lg p-3 border border-purple-200">
                <span className="text-xs font-medium text-purple-700 uppercase block mb-1">SDV Percentage</span>
                <EditableText
            ...
            {sdvStrategy.sdv_timing && (
              <div className="bg-white rounded-lg p-3 border border-purple-200">
                <span className="text-xs font-medium text-purple-700 uppercase block mb-1">SDV Timing</span>
                <EditableText
          ...
          {sdvStrategy.critical_data_points?.length > 0 && (
            <div className="mt-4">
              <span className="text-xs font-medium text-purple-700 uppercase block mb-2">Critical Data Points (100% SDV)</span>
              <div className="flex flex-wrap gap-2">
                {sdvStrategy.critical_data_points.map((point: string, idx: number) => (
                  <span key={idx} className="text-xs bg-purple-100 text-purple-700 px-3 py-1.5 rounded-full font-medium">
                    {point}
                  </span>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {queryMgmt.resolution_target_days && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <span className="text-xs font-medium text-blue-700 uppercase block mb-1">Resolution Target</span>
                <span className="font-semibold text-blue-900">{queryMgmt.resolution_target_days} days</span>
              </div>
            )}
            {queryMgmt.escalation_threshold_days && (
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <span className="text-xs font-medium text-amber-700 uppercase block mb-1">Escalation Threshold</span>
                <span className="font-semibold text-amber-900">{queryMgmt.escalation_threshold_days} days</span>
              </div>
            )}
          </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {design.external_data_sources.map((source: string, idx: number) => (
                  <span key={idx} className="inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full font-medium">
            ...
            {interimLocks.map((lock: any, idx: number) => (
              <div key={lock.id || idx} className="p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center text-sm font-bold text-amber-700">
                      {idx + 1}
                    </div>
                    <div>
                      <EditableText
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sections.map(({ key, label, icon: Icon, color }) => {
          ...
          const colorClasses: Record<string, { bg: string; border: string; text: string; iconBg: string }> = {
            blue: { bg: "from-blue-50 to-indigo-50", border: "border-blue-200", text: "text-blue-900", iconBg: "bg-blue-600" },
            purple: { bg: "from-purple-50 to-violet-50", border: "border-purple-200", text: "text-purple-900", iconBg: "bg-purple-600" },
            green: { bg: "from-green-50 to-emerald-50", border: "border-green-200", text: "text-green-900", iconBg: "bg-green-600" },
            amber: { bg: "from-amber-50 to-orange-50", border: "border-amber-200", text: "text-amber-900", iconBg: "bg-amber-600" },
            red: { bg: "from-red-50 to-rose-50", border: "border-red-200", text: "text-red-900", iconBg: "bg-red-600" },
          };
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {archival.archival_format.map((format: string, idx: number) => (
              <span key={idx} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-indigo-100 text-indigo-700 rounded-full">
