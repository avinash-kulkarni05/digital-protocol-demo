              ...
              <div>
                ...
                <div className="space-y-2">
                  {modifications.modificationTriggers.map((trigger: any, tidx: number) => (
                    <div key={tidx} className="bg-amber-50 rounded-lg p-3 border border-amber-200">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-medium text-amber-900">{trigger.condition}</p>
                          {trigger.toxicityGrade && (
                            <span className="text-xs text-amber-700">Grade {trigger.toxicityGrade}</span>
                          )}
                        </div>
                        <span className={cn(
                          "text-xs px-2 py-0.5 rounded-full font-medium",
                          trigger.action === "Discontinue" ? "bg-red-100 text-red-700" :
                          trigger.action === "Hold" ? "bg-amber-100 text-amber-700" :
                          trigger.action === "Reduce" ? "bg-blue-100 text-blue-700" :
                      ...
                      {trigger.holdCriteria && (
                        <p className="text-xs text-amber-700 mt-1">Hold criteria: {trigger.holdCriteria}</p>
                      )}
                      {trigger.targetLevel && (
                        <p className="text-xs text-amber-700 mt-1">Target level: {trigger.targetLevel}</p>
                      )}
                    </div>
                  ))}
                </div>
