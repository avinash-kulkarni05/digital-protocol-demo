  ...
  if (typeof concept === 'string') {
    return (
      <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
        <span className="text-sm text-purple-900">{concept}</span>
      </div>
    );
  }
    ...
    <div className="bg-purple-50 rounded-lg p-4 border border-purple-200 space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          {domain && (
            <span className="inline-flex items-center px-2 py-1 text-xs font-bold bg-purple-600 text-white rounded">
              {domain}
            </span>
          )}
          {cdiscCode && (
            <span className="inline-flex items-center px-2 py-1 text-xs font-mono bg-purple-100 text-purple-800 rounded border border-purple-300">
              {cdiscCode}
            </span>
          )}
        </div>
      ...
      {conceptName && (
        <div>
          <span className="text-xs font-medium text-purple-600 uppercase block mb-1">Concept Name</span>
          <span className="text-sm font-medium text-purple-900">{conceptName}</span>
        </div>
      )}
      ...
      {rationale && (
        <div>
          <span className="text-xs font-medium text-purple-600 uppercase block mb-1">Rationale</span>
          <span className="text-sm text-purple-800 italic">{rationale}</span>
        </div>
      )}
    </div>
  );
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-5">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h4 className="font-bold text-blue-900 text-lg">Administration Schedule</h4>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {administration.compliance_threshold && (
              <div className="bg-white/70 rounded-lg p-3 border border-blue-200">
                <span className="text-xs font-medium text-blue-700 uppercase block mb-1">Compliance Threshold</span>
                <EditableText
            ...
            {administration.completion_requirements && (
              <div className="bg-white/70 rounded-lg p-3 border border-blue-200">
                <span className="text-xs font-medium text-blue-700 uppercase block mb-1">Completion Requirements</span>
                <EditableText
          ...
          {administration.timepoints?.length > 0 && (
            <div className="mt-4">
              <h5 className="text-sm font-medium text-blue-800 mb-3">Assessment Timepoints ({administration.timepoints.length})</h5>
              <div className="flex flex-wrap gap-2">
                {administration.timepoints.map((tp: any, idx: number) => (
                  <span key={idx} className="inline-flex items-center px-3 py-1 bg-white/80 text-blue-900 text-sm rounded-full border border-blue-200">
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                  ...
                  <div className="space-y-2">
                    {instrument.scoring_domains.map((domain: any, domIdx: number) => (
                      <div key={domIdx} className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                        <span className="font-medium text-purple-900">{domain.domain_name || domain.name || domain}</span>
                        {domain.scoring_method && <span className="text-xs text-purple-700 ml-2">({domain.scoring_method})</span>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {instrument.biomedicalConcept && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-purple-700">CDISC Biomedical Concept</span>
                  </div>
