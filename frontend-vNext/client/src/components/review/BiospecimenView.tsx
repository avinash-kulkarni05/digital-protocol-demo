...
function SummaryHeader({ data }: { data: any }) {
  ...
  const rawVolume = data?.volume_summary?.total_blood_volume;
  const totalVolume = typeof rawVolume === 'object' && rawVolume !== null && 'value' in rawVolume
    ? `${rawVolume.value ?? ''}${rawVolume.unit ? ' ' + rawVolume.unit : ''}`.trim() || "N/A"
    : (rawVolume || "N/A");

  const rawMax = data?.volume_summary?.per_visit_maximum;
  const perVisitMax = typeof rawMax === 'object' && rawMax !== null && 'value' in rawMax
    ? `${rawMax.value ?? ''}${rawMax.unit ? ' ' + rawMax.unit : ''}`.trim() || "N/A"
    : (rawMax || "N/A");
